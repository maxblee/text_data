...
def distance_heatmap(
    ...
    data = [
        {left_name: i, right_name: j, "Similarity": distance_matrix[i][j]}
        for i in index
        for j in cols
    ]
    return (
        ...
        .encode(
            x=f"{left_name}:O",
            y=f"{right_name}:O", color="Similarity:Q",
...
def display_multiple_searches(
    ...
    for count, corpus in enumerate(corpuses):
        ...
        if max_results is not None and total_max is not None:
            new_results = min(max_results, total_max - num_results)
        else:
            new_results = None
...
def risk_ratio(
    ...
    adjusted_left = (left_corpus.word_frequency(word) + 1) / left_corpus.vocab_size
    adjusted_right = (right_corpus.word_frequency(word) + 1) / right_corpus.vocab_size
...
def get_word_scores(
    corpuses: List[Corpus],
    scoring_func: Callable[[Corpus, List[Corpus], Optional[Corpus], str], Optional[float]],
    background_corpus: Optional[Corpus] = None
    ...
    for i, corpus in enumerate(corpuses):
        ...
        for word in combined_vocab:
            word_score = scoring_func(corpus, remaining_corpuses, background_corpus, word)
            if word_score is not None:
...
def tfidf(
    ...
    idf = (len(other_corpuses) + 1) / doc_count
...
def display_difference_graph(
    ...
    import altair as alt
    data = []
    word_diffs = get_word_differences(left_corpus, right_corpus, scoring_func, background_corpus)
...
def display_top_differences(
    ...
    Args:
        ...
        num_results: The number of results you want to return from each corpus.
            If None, it will return all results.
    """
    word_diffs = get_word_differences(left_corpus, right_corpus, scoring_func, background_corpus)
    top_scores = word_diffs[:num_results]
    bottom_results = word_diffs[-num_results:]
    return display_notable([top_scores, bottom_results], ["High Scores", "Low Scores"], num_results)
...
def _display_score_table(scores: List[Tuple[float, str]]) -> str:
    ...
    html += "".join([
        ...
        for count, (word, score) in enumerate(scores)
    ])
    html += "</tbody></table>"
    return html